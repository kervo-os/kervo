...
Mode 3 はブートストラップ用チャンネルです: キャプチャするエージェントが
まだ働いていないリポジトリで空のスロットを埋める用途。Mode 2 のセッション
キャプチャが生きているなら env を設定しないでください — artifact だけを
読む推論は意図ではなく履歴を読み、ほとんどレビューノイズを増やすだけです
(実リポジトリで計測)。
...
提案と同じく関門になります。キューを人間が扱える規模に保つ規範が二つ:
**コーパスではなく結論のみ** — ファイルにあるものはファイルに置き、証拠
として引用 — そして**バックプレッシャ**: 判定待ち提案が既に 12 件ある
ソースは、人間が判定するまでこれ以上追加できません。この形式に state フィールドがないのは設計です:
...
人間は判定だけを行います。** そして会話がレビューサーフェスです: 人間が
セッション内で事実に同意すれば、エージェントがその言葉を引用して判定を
共に中継します — キューには誰も見ていないものだけが残ります。 `kervo review` がその判定サーフェスです —
...
そして実際の本番モノレポで (2026-07-06、そのリポジトリ自身の台帳より):

| 計測したもの | 結果 |
|---|---|
| write-back パイロット: capture → 台帳 → compile → 新規コンシューマ | オンボーディング回答 **5.5/10 → 9.5/10**、コスト不変(ツール呼び出し 1 回) |
| トラストラベルのコンシューマ到達 | 消費エージェントが指示なしに自分の回答を `[generated]` と明示 |
| Mode 3 バックエンド提案の採点 | goal C+ / risk D → ブートストラップ用に再配置(上記参照) |

事前登録の完全な再実行はカレンダーではなくボリュームゲート
(セッション 10 + 判定済み write-back 10)で行います。

## キャプチャ: フックの接続

ライブキャプチャが台帳と内蔵計測カウンタを養います。対話的な `kervo init`
がこの配線を代行します(スクリプトでは `-hooks yes`);手動なら
...
28 日間のアクティビティ・トラスト構成・プロジェクト概要(宣言された
コマンド、最近の変更、モジュール)・コミット履歴で証明された結合度・
実際に接続されたアダプタ — キーボードファーストのインライン判定
(`1`–`9` でリポジトリを開く、`j`/`k` 移動、`v`/`s`/`d` 判定、`?` キー
一覧)付きで、各判定はそのリポジトリ自身の台帳に記録されます。キューの下のナレッジ
ビューは verified・observed 項目の全文を — 主張を先に、証拠付きで —
表示し、退役した項目は理由と共に残ります。

<p align="center"><img src="assets/dash-fleet.png" width="860" alt="kervo dash — 全ワークスペース、保留判定、アクティビティ"></p>

<p align="center"><img src="assets/dash-detail.png" width="860" alt="kervo dash — ワークスペース詳細: 判定、概要、結合度"></p>真実は
