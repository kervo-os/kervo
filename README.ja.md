...
`post-commit`・`post-merge` フックを配線し、ローカルコミットも **pull で
入ってくるコミット**もデフォルトで artifact を更新します。git フックは
マシンローカルなので、チームメイトは最初の `kervo compile` 一回で自分の
マシンが配線されます。自分で書いたフックは決して書き換えません(我々の
フックを自分のものに置き換えることがオプトアウトです)。
</details>
