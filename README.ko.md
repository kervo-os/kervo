...
**결정이 CI를 게이트합니다.** 결정에 앵커를 달면 —
`kervo capture -type decision -body "Payments는 Q3까지 legacy gateway
유지" -anchor "services/payments/**"` — verified 결정이 다스리는 경로를
건드리는 PR마다 `kervo check`가 diff 인라인으로 경고합니다(GitHub
annotations, 봇 코드 0줄). 기본은 advisory: 충돌하는 PR은 의도된 번복일
수 있으니, 경고문이 루프를 가르칩니다 — 사유와 함께 deprecate하고 새
결정을 capture. 앵커 경로가 트리에서 사라지면 stale 후보로 표시되고, 판정 이후에도
앵커 밑의 코드가 계속 움직인 verified 결정에는 재확인을 요구합니다 —
나이 타이머가 아니라 증거 기반 무효화. 기록되지 않은 번복은 churn으로
도착하고, 그 churn을 시스템이 지켜봅니다.

```yaml
- run: git fetch origin main
- run: kervo check -base origin/main   # 차단하려면 -strict
```
...
- **A−C = +29.2%p** — 사전 등록 기준(≥20%p) 충족 (95% CI 6.9–51.5%p —
  통계적 증명이 아니라, 복제 킷이 공개된 강한 사전 등록 신호입니다).
  프로그램 전체에서 발생한 실제 오염 감염(3건)은 전부 약한 소비자
  모델의 C 암에서 나왔습니다.
- 프로그램 전체 54개 응답에서 A 암은 오염된 주장에 단 1점도 잃지
  않았습니다. 위 표에서 B가 A와 동률이라는 것은, 종합 효과의 본체가
  라벨 단독이 아니라 **취급 정책** — 낡은 것은 분리, 퇴역은 제외 —
  이라는 뜻입니다.
- 라벨의 몫은 전염이 덮치는 곳에서 드러납니다: 앞선 혼합 조건 런에서
  무라벨 암은 거짓 하나가 발견되면 *참인* 사실까지 연좌제로 배척했습니다.
  코드가 반증할 수 있는 거짓은 에이전트가 스스로 막습니다 — 라벨은 코드
  밖의 진실(결정, 제약, 맥락)이 함께 매도당하는 것을 막고, 소비자가
  약할수록 보호 효과는 커집니다.
...
| `kervo capture -type <t> -body <md> -evidence <e> [-anchor <glob>…]` | 관찰 기록 (중복 제거 + 역압 가드); 앵커는 이 관찰이 다스리는 경로 |
| `kervo trust -id <접두> -to verified\|stale\|deprecated -reason <사유>` | ID로 판정 (스크립트용 프리미티브) |
| `kervo review` | 터미널 검토 큐 — 하나씩 판정 |
| `kervo check [-base <ref>] [-strict]` | diff 게이트: 이 변경이 건드리는 verified 앵커 결정은? |
