...
제안과 똑같이 관문이 됩니다. 큐를 사람이 감당하게 하는 규범 둘:
**코퍼스가 아니라 결론만** — 파일에 있는 건 파일에 두고 증거로 인용 —
그리고 **역압**: 판정 대기 제안이 이미 12건인 소스는 사람이 판정하기
전까지 더 넣을 수 없습니다. 이 형식에 state 필드가 없는 건 설계입니다:
...
판정만 합니다.** 그리고 대화가 곧 검토 표면입니다: 사람이 세션에서 사실에
동의하면 에이전트가 그 발언을 인용해 판정을 함께 중계합니다 — 큐에는
아무 사람도 못 본 것만 남습니다. `kervo review`가 그 판정 표면입니다 — 결정을 기다리는
모든 것을 하나씩 보여주는 검토 큐.
...
라이브 캡처가 장부와 내장 측정 카운터를 먹입니다. 대화형 `kervo init`이
이 배선을 대신 해줍니다(스크립트에선 `-hooks yes`); 직접 하려면 프로젝트의
`.claude/settings.json`에 추가하세요 (훅은 프로젝트 디렉토리에서
...
v0.19.x — 릴리스는 CI 관문을 거쳐 이유가 있을 때만 나가며, 전부
`CHANGELOG.md`에 기록됩니다. 실제 운영 레포에서 가동 중: write-back
파일럿이 온보딩 답변 **5.5/10 → 9.5/10, 도구 호출 1회**를 실측했고,
첫 크로스-벤더 write-back(`AGENTS.md`에서 프로토콜을 읽은 Codex 세션)이
실장부에 들어와 있습니다. 실험 프로토콜과 원자료는
[kervo-os/experiments](https://github.com/kervo-os/experiments)에 있으며,
사전등록 플라이휠 재실행은 달력이 아니라 볼륨 게이트(세션 10 + 판정된
write-back 10)에서 실행됩니다.
